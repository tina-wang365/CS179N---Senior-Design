...
public class AIScript : MonoBehaviour
{
	...
	//ADD A JUMP RANGE to detect how far platform is
	//to NOT over jump
	...
	public Vector3 currLoc; //current location
	public Vector3 nextLoc; //next location
	...
	// FixedUpdate function BEFORE INVI SCRIPT
	void FixedUpdate()
	{
		Debug.Log ("AI FU EXECUTION!!!!!!!!!!!!!!!");
...
//		Debug.Log(rigidbody.position.x + " jump to: " + nextLoc.x);
		...
		if (onGround && !(invi.inviGround))
			actionJump = true;

		Debug.Log ("Jump = " + actionJump);
...
//				actionJump = false;
//		}
//		else if(other.gameObject.name.Equals("Platform") && other.gameObject.GetComponent<MeshRenderer>().material.color.Equals(Color.white))
//		{
//
//			isWhite = true;
//			onGround = true;
...
//			                                            rigidbody.transform.position.z);
			Debug.Log ("ON FLOOR");
			onGround = true;
			//if(invi.inviJump && invi.inviGround)
			//	actionJump = false;
		}
...
//	}
	...
	public void init()
	{
		...
		createInvi = false;
		invisPlayer = GameObject.Find("inviPlayer");
		invi = invisPlayer.GetComponent<inviScript> ();
//
